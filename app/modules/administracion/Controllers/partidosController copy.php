...
class Administracion_partidosController extends Administracion_mainController
{
	...
	public function actualizarPuntosAction()
	{
		...
		$usuarios = $resultadosModel->getUsuarios("", "");
		...
		/* $AllResults = $resultadosModel->getList();
		$resultados_array = array();
		foreach ($AllResults as $key => $value) {
			$resultados_array[$value->usuario] = $value;
		}
 */
		...
		foreach ($usuarios as $key => $usuario) {
			...
			$resultados = $resultadosModel->getList("usuario = $usuarioId", "fecha DESC");
			...
			/* $e1 = $cuartos->e1;
			if ($e1 == $clasificado_cuartos_e1) {
				$puntos += $clasificadoCuartos;
				$otros++;
			}
			$e2 = $cuartos->e2;
			if ($e2 == $clasificado_cuartos_e2) {
				$puntos += $clasificadoCuartos;
				$otros++;
			}
			$e3 = $cuartos->e3;
			if ($e3 == $clasificado_cuartos_e3) {
				$puntos += $clasificadoCuartos;
				$otros++;
			}
			$e4 = $cuartos->e4;
			if ($e4 == $clasificado_cuartos_e4) {
				$puntos += $clasificadoCuartos;
				$otros++;
			}
			$e5 = $cuartos->e5;
			if ($e5 == $clasificado_cuartos_e5) {
				$puntos += $clasificadoCuartos;
				$otros++;
			}

			$e6 = $cuartos->e6;
			if ($e6 == $clasificado_cuartos_e6) {
				$puntos += $clasificadoCuartos;
				$otros++;
			}

			$e7 = $cuartos->e7;
			if ($e7 == $clasificado_cuartos_e7) {
				$puntos += $clasificadoCuartos;
				$otros++;
			}

			$e8 = $cuartos->e8;
			if ($e8 == $clasificado_cuartos_e8) {
				$puntos += $clasificadoCuartos;
				$otros++;
			}
 */
	...
	/* public function actualizarPuntosAction()
	{
		...
		$resultadosModel = new Administracion_Model_DbTable_Resultados();
		$partidosModel = new Administracion_Model_DbTable_Partidos();
		$config = $this->getConfig(1); // Método separado para obtener la configuración

		$usuarios = $usuariosModel->getList("", "");
		$partidos = $partidosModel->getList("", ""); // Suponiendo que podemos cargar todos los partidos de una vez
		$partidos_array = [];
		foreach ($partidos as  $value) {
			$partidos_array[$value->id] = $value;
		}
		// print_r($partidos_array["1"]);
		foreach ($usuarios as $usuario) {
			$puntos = 0;
			$resultados = $resultadosModel->getList("usuario = {$usuario->user_id}", "");
			//print_r($resultados);
			foreach ($resultados as $resultado) {

				$partido = $partidos_array[$resultado->partido]; // Acceso a un array en lugar de a la base de datos
				// print_r($resultado->partido);

				//print_r($partido);
				if ($partido->fase == 1) { // Suponiendo que solo hay una fase para simplificar
					list($puntosPorResultado, $aciertos) = $this->calcularPuntos($partido, $resultado, $config);
					$puntos += (int)$puntosPorResultado;
				}
			}

			 echo "Usuario {$usuario->user_id} Puntos: $puntos<br/>";
			// Aquí podrías actualizar la base de datos con los puntos totales del usuario si es necesario
		}
	}
	...
	{
		$configModel = new Administracion_Model_DbTable_Config();
		return $configModel->getById($id);
	}

	private function calcularPuntos($partido, $resultado, $config)
	{
		$puntos = 0;
		$aciertos = ['marcadores' => 0, 'ganadores' => 0];

		// Asumimos que no hay valores vacíos
		if ($partido->valor1 == $resultado->valor1 && $partido->valor2 == $resultado->valor2) {
			$aciertos['marcadores']++;
			$puntos += (int)$config->config_gruposmarcador;
		}

		if ($partido->ganador == $resultado->ganador) {
			if ($partido->ganador == -1) {
				$puntos += (int)$config->config_gruposempate;
			} else {
				$puntos += (int)$config->config_gruposequipo;
			}
			$aciertos['ganadores']++;
		}
