	...
	<form class="text-left" enctype="multipart/form-data" method="post" action="<?php echo $this->routeform;?>"  data-bs-toggle="validator">
		<div class="content-dashboard">
			...
			<div class="row">
				...
				<div class="col-3 form-group">
					<label class="control-label">goleador</label>
					<label class="input-group">
						<div class="input-group-prepend">
							<span class="input-group-text input-icono  fondo-morado " ><i class="far fa-list-alt"></i></span>
						</div>
						<select class="form-control" name="goleador"  required >
							<option value="">Seleccione...</option>
							<?php foreach ($this->list_goleador AS $key => $value ){?>
								<option <?php if($this->getObjectVariable($this->content,"goleador") == $key ){ echo "selected"; }?> value="<?php echo $key; ?>" /> <?= $value; ?></option>
							<?php } ?>
						</select>
					</label>
					<div class="help-block with-errors"></div>
				...
				<div class="col-3 form-group">
					...
					<label class="input-group">
						...
						<select class="form-control" name="valla"  required >
							<option value="">Seleccione...</option>
							<?php foreach ($this->list_valla AS $key => $value ){?>
								<option <?php if($this->getObjectVariable($this->content,"valla") == $key ){ echo "selected"; }?> value="<?php echo $key; ?>" /> <?= $value; ?></option>
							<?php } ?>
						</select>
					</label>
					<div class="help-block with-errors"></div>
